...
werd <- werd %>% distinct(reform_description, .keep_all = TRUE)  ##11,108 reforms

#After translation
load("werdV3CleanTM.rda")
```
...
View(ref.toks) 
# after reviewing the term frequency matrix, we decided to further trim the term-document matrix to preclude tokens below a minimum of overall frequency of 9 and document frequency of 5, to increase the likelihood that words included in the topics carry the most meaning to describe it.
...
save(ref.stm, file = "stmV3_final.rda")

dropped_indices <- c(1595, 2908, 8, 147, 162, 153, 144, 8846, 8851, 8837, 4352, 9509, 16636, 13500, 3408, 5290, 4558, 4159, 13535, 13538, 5476, 5405, 6692, 6895) ## 24 reforms were dropped

werd_without_dropped <- werd[!werd$reform_id %in% dropped_indices, ] ##11,084 reforms

save(werd_without_dropped, file = "werd_without_dropped.rda")
```

```{r methods to select K}
load("stmV3_final.rda")
...
kresult_v3 <- searchK(documents=ref.stm$documents, 
                   ...
                   K = seq(30,50, 5), 
                   verbose = F) 
save(kresult_v3 , file = "kresult_v3.rda")
beep()
load("kresult_v3.rda")
...
load("werd_without_dropped.rda")
findThoughts(model = mod30,
             texts = werd_without_dropped$reform_description,
             topics = 23,
             n = 10)
write_xlsx(werd_without_dropped, path = "werd_w_o_dropped.xlsx")
...
colnames(topic_scores) <- paste("topic", 1:30)

WERD_V3_TopicScores <- cbind(werd_without_dropped,topic_scores)
write.csv(WERD_V3_TopicScores, file = "WERDV3_withTopicScores.csv")
```
